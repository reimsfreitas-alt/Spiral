...
CREATE TABLE IF NOT EXISTS knowledge_repository (
  ...
  governance_level INT NOT NULL DEFAULT 1 CHECK (governance_level BETWEEN 0 AND 4),
  version INT NOT NULL DEFAULT 1 CHECK (version >= 1),
  lineage JSONB NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (tenant_id, subject)
);
CREATE INDEX IF NOT EXISTS idx_knowledge_tenant_subject ON knowledge_repository(tenant_id, subject);
...
CREATE POLICY tenant_isolation_policy_knowledge ON knowledge_repository FOR ALL
USING (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::UUID)
WITH CHECK (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::UUID);

DROP POLICY IF EXISTS tenant_isolation_policy_idempotency ON idempotency_ledger;
CREATE POLICY tenant_isolation_policy_idempotency ON idempotency_ledger FOR ALL
USING (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::UUID)
WITH CHECK (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::UUID);

DROP POLICY IF EXISTS tenant_isolation_policy_branches ON branch_registry;
CREATE POLICY tenant_isolation_policy_branches ON branch_registry FOR ALL
USING (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::UUID)
WITH CHECK (tenant_id = NULLIF(current_setting('app.current_tenant_id', true), '')::UUID);
