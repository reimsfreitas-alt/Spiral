...
export interface AppendOptions {
  causationId?: string; idempotencyKey?: string; authorizationId?: string;
}
export class EventFabric {
  constructor(private readonly pool: Pool) {}
  public async append(tenantId:string, sourceModule:string, eventType:string, correlationId:string,
    payload:Record<string,unknown>, confidence=1.0, options:AppendOptions={}):Promise<string>{
    ...
    try{
      ...
      if(options.idempotencyKey){
        ...
        await client.query(
          "INSERT INTO idempotency_ledger(idempotency_key,tenant_id,authorization_id,status) VALUES($1,$2,$3,'ACCEPTED') ON CONFLICT (idempotency_key) DO NOTHING",
          [options.idempotencyKey,tenantId,options.authorizationId]);
      }
      const eventPayload=options.idempotencyKey?{...payload,idempotency_key:options.idempotencyKey}:payload;
      const result=await client.query<{event_id:string}>(
        `INSERT INTO event_log(tenant_id,source_module,event_type,correlation_id,causation_id,payload,confidence)
      ...
      await client.query("COMMIT"); return result.rows[0].event_id;
  ...
  public async replayByCorrelation(tenantId:string,correlationId:string):Promise<SpiralEventRecord[]>{
    ...
    try{
      ...
      const result=await client.query<SpiralEventRecord>(
        `SELECT event_id,tenant_id,source_module,event_type,correlation_id,causation_id,payload,confidence,schema_version,created_at
           FROM event_log WHERE tenant_id=$1 AND correlation_id=$2 ORDER BY created_at ASC,event_id ASC`,
